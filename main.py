...
# Load data
data = pd.read_csv(r'C:\Users\mamid\Downloads\extended_stock_data.csv', parse_dates=['Date'])
data.set_index('Date', inplace=True)

# Select target
target_column = st.selectbox("Select the target column for prediction", options=['AMZN', 'DPZ', 'BTC', 'NFLX'])
X = data[['DPZ', 'BTC', 'NFLX']]
...
# Ensure prediction data shape matches model input
if future_days <= len(X):
    future_predictions = model.predict(X.iloc[-future_days:])
else:
    st.warning("Not enough past data for the selected future prediction window.")
    future_predictions = [None] * future_days  # Placeholder
